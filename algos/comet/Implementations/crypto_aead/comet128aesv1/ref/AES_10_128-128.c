...
const u8 emulated_aesenc_rijndael_sbox[256] = {
...
const u32 emulated_aesenc_enc_table_0[256] =
{
...
const u32 emulated_aesenc_enc_table_1[256] =
{
...
const u32 emulated_aesenc_enc_table_2[256] =
{
...
const u32 emulated_aesenc_enc_table_3[256] =
{
...
void emulated_aesenc_substitute_bytes(
				 u32 *data)
{
	int i;
	u8 *ptr = (u8 *)data;
...
void emulated_aesenc_row_shifting(
			u32 *data)
{
	u8 byte_order[16];
	byte_order[0]  = 0;
...
static const u32 rcon[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

void AES_128_Key_Expansion(u32 *ks, const u8 *userkey){
	
    const u8 *key = userkey;
    u32* w = ks;
    const int Nk = 4;
    const int Nb = 4;
    const int Nr = ROUNDS;
    ...
    while (i < Nk)
    {
        w[i] = (u32)key[4*i] ^ ((u32)key[4*i+1]<<8) ^ ((u32)key[4*i+2]<<16) ^ ((u32)key[4*i+3]<<24);
        i++;
    }
    i = Nk;
    while (i < Nb * (Nr + 1))
    {
        ...
        {
            temp = 
                ((u32)emulated_aesenc_rijndael_sbox[(temp      ) & 0xff] << 24) ^
				((u32)emulated_aesenc_rijndael_sbox[(temp >>  8) & 0xff] << 0) ^
				((u32)emulated_aesenc_rijndael_sbox[(temp >> 16) & 0xff] << 8) ^
				((u32)emulated_aesenc_rijndael_sbox[(temp >> 24)       ] << 16) ^
				rcon[i/Nk - 1];
        }
        w[i] = w[i - Nk] ^ temp;
        i++;
...
{
	//copy input to 32-bit registers
	u32 in[4] = { 0 };
	u32 ks[4*(ROUNDS+1)] = { 0 };
	u32 out[4] = { 0 };
	
	for(u8 i=0; i<4; i++){
		in[i] = (u32)pt[i*4+3]<<24 | (u32)pt[i*4+2]<<16  | (u32)pt[i*4+1]<<8  | (u32)pt[i*4] ;
	}
	
	//key expansion
	AES_128_Key_Expansion(ks, key);
	...
	u32 key_ctr = 0;
    s0 = in[0]^ks[key_ctr];
    s1 = in[1]^ks[key_ctr+1];
    s2 = in[2]^ks[key_ctr+2];
    s3 = in[3]^ks[key_ctr+3];
    key_ctr+=4;
	...
	//convertig output to 8-bit
	for(u8 i=0; i<4; i++){
		for(u8 j=0; j<4; j++){
			ct[i*4+j] = (out[i]>>(j*8)) & 0xFF;
		}
	}
	return;
}
