/* Reference implementation of the sLiSCP-light256 permutation
   Written by:
...
/*
   *sliscp_permutation256r9: 9-round sliscp-light permutation of width 256 bits
...
void sliscp_permutation256r9 ( uint8_t *input )
{
	...
	for ( i = 0; i < NUMSTEPSR9; i++ )
	{
		...
		// Add step constant: SC[0]^x0^F(x1)
		...
		// Add step constant: SC[1]^x2^F(x3)
		for ( j = 0; j < SIMECKBYTES-1; j++ )
...
void sliscp_permutation256r18 ( uint8_t *input )
{
        ...
        for ( i = 0; i < NUMSTEPSR18; i++ )
        {
                ...
                for ( j = 0; j < SIMECKBYTES-1; j++ )
                        tmp_pt[SIMECKBYTES+j] = tmp_pt[SIMECKBYTES+j]^(0xff);
                tmp_pt[2*SIMECKBYTES-1] = tmp_pt[2*SIMECKBYTES-1]^SC2_256[i]; // x1' = SC[1]^x2^F(x3)//
                
                for ( j = 0; j < SIMECKBYTES; j++ )
                        tmp_pt[2*SIMECKBYTES + j ] = simeck_inp[j]; // x2' = F(x3)//
                
                for ( j = 0; j < SIMECKBYTES; j++ )
                        tmp_pt[3*SIMECKBYTES + j ] = tmp_block[j]; // x3' = SC[0]^x0^F(x1)//
